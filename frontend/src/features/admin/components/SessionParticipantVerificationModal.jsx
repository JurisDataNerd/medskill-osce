...
export default function SessionParticipantVerificationModal({
  isOpen,
  onClose,
  sessionId,
  sessionTitle = "Sesi Ujian OSCE",
  onUpdate,
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      <div className="flex flex-col w-full max-w-5xl max-h-[90vh] bg-white shadow-2xl rounded-3xl overflow-hidden border border-slate-200">
        {/* Top Modal Header */}
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-black leading-tight">
                  Penugasan & Verifikasi Peserta
                </h2>
                <span className="rounded-full bg-blue-400/20 border border-blue-400/40 text-blue-300 text-[10px] font-black px-2.5 py-0.5">
                  {sessionTitle}
                </span>
              </div>
              <p className="text-xs text-slate-300 font-medium mt-0.5">
                Plotting urutan rotasi sirkuit dan persetujuan (approval) pendaftaran peserta.
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-6 py-3">
          ...
          <div className="relative min-w-[220px]">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-white pl-9 pr-3 py-1.5 text-xs font-medium focus:border-blue-500 focus:outline-none"
              value={searchQuery}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-100/60">
          {loading ? (
            <div className="flex h-64 items-center justify-center rounded-2xl bg-white border border-slate-200 shadow-2xs">
              <Loader2 className="h-8 w-8 animate-spin text-blue-600" />
            </div>
          ) : filteredParticipants.length === 0 ? (
            <div className="flex h-64 flex-col items-center justify-center rounded-2xl bg-white border border-slate-200 text-center p-6 space-y-2">
              <Users size={36} className="text-slate-300" />
              <h4 className="text-sm font-extrabold text-slate-800">Tidak ada pendaftar peserta</h4>
              <p className="text-xs text-slate-500 max-w-sm">
                Belum ada calon peserta yang mendaftar pada sesi ujian ini dengan filter aktif.
              </p>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b bg-slate-50 text-slate-500 font-extrabold uppercase text-[10px] tracking-wider">
                    ...
                    <th className="p-3.5">Awal Stase & Rotasi</th>
                    <th className="p-3.5">Aktivitas Login</th>
                        ...
                        <td className="p-3.5">
                          <div className="flex items-center gap-3">
                            <div className="relative shrink-0">
                              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-100 text-xs font-black text-blue-800 border border-blue-200">
                            ...
                            {isProcessing ? (
                              ...
                              <>
                                {item.status === "pending" && (
                                  <>
                                    <button
                                      ...
                                      className="inline-flex items-center gap-1 rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white transition hover:bg-emerald-700 active:scale-95 shadow-xs"
                                    >
                                    ...
                                    <button
                                      ...
                                      className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-red-600 transition hover:bg-red-50"
                                    >
                                      <XCircle size={13} />
                                      Tolak
                                    </button>
                                ...
                                {item.status === "approved" && (
                                  <button
                                    onClick={() => handleReject(item.id)}
                                    className="inline-flex items-center gap-1 rounded-xl border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-bold text-red-700 transition hover:bg-red-100"
                                ...
                                {item.status === "rejected" && (
                                  <button
                                    onClick={() => handleApprove(item.id)}
                                    className="inline-flex items-center gap-1 rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-xs font-bold text-emerald-800 transition hover:bg-emerald-100"
                                  >
                                    <CheckCircle2 size={13} />
                                    Setujui Kembali
                                  </button>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 bg-white px-6 py-4">
          <p className="text-xs text-slate-500 font-semibold">
            Status approval menentukan akses peserta untuk masuk ke halaman ujian OSCE.
          </p>
          <button
            onClick={onClose}
            className="rounded-xl border border-slate-200 bg-white px-5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 transition shadow-2xs"
          >
            Selesai Verifikasi
          </button>
        </div>
