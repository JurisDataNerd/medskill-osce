...
export default function AdminAuxiliaryExamBuilder({
  ...
  const handleUpdateConfig = (targetId, field, value) => {
    const updated = configs.map((c) => {
      const currentId = c.itemId || c.item_id || c.id;
      if (currentId === targetId) {
        const itemCopy = { ...c, [field]: value };
        if (field === "imageUrl") {
          itemCopy.imageUrl = value;
          itemCopy.image_url = value;
          itemCopy.file_url = value;
        }
        if (field === "reportText" || field === "report_text") {
          itemCopy.reportText = value;
          itemCopy.report_text = value;
        }
        return itemCopy;
      }
      return c;
  ...
  const handleRemoveItem = (targetId) => {
    const updated = configs.filter((c) => (c.itemId || c.item_id || c.id) !== targetId);
    if (onChangeConfigs) {
      onChangeConfigs(updated);
    }
  };
      ...
      {configs.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {configs.map((item, idx) => {
            const idVal = item.itemId || item.item_id || item.id || `aux-${idx}`;
            const imgUrl = item.imageUrl || item.image_url || item.file_url || "";
            const reportVal = item.reportText || item.report_text || "";
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-slate-900">
                        {item.name || item.title || "Berkas Penunjang"}
                      </h4>
                  ...
                  <button
                    type="button"
                    onClick={() => handleRemoveItem(idVal)}
                  ...
                  {imgUrl ? (
                    <div className="flex items-center gap-3">
                      <div className="relative group h-16 w-24 shrink-0 rounded-lg overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center">
                        {imgUrl.includes("drive.google.com") || imgUrl.includes("docs.google.com") ? (
                          ...
                          <img
                            src={imgUrl}
                            alt={item.name || "Berkas"}
                            className="h-full w-full object-cover"
                          />
                        )}
                        <button
                          type="button"
                          onClick={() => handleUpdateConfig(idVal, "imageUrl", "")}
                      ...
                      <div className="flex-1 space-y-1">
                        <p className="text-[11px] font-semibold text-emerald-700 flex items-center gap-1">
                          ...
                          {imgUrl.includes("drive.google.com") ? "Google Drive Link Terlampir (Iframe Supported)" : "Berkas Terlampir"}
                        </p>
                        <input
                          type="text"
                          placeholder="URL Gambar / Link Google Drive..."
                          value={imgUrl}
                          onChange={(e) => handleUpdateConfig(idVal, "imageUrl", e.target.value)}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              reader.onload = (evt) => {
                                handleUpdateConfig(idVal, "imageUrl", evt.target.result);
                              };
