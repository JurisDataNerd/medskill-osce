...
export async function createSession(sessionPayload, stationsPayload = []) {
  ...
  if (stationsPayload.length > 0) {
    const formattedStations = stationsPayload.map((st, idx) => ({
      ...
      answer_key_wdx: st.answer_key_wdx || null,
      answer_key_ddx1: st.answer_key_ddx1 || null,
      answer_key_ddx2: st.answer_key_ddx2 || null,
      answer_key_ddx3: st.answer_key_ddx3 || null,
      answer_key_ddx: st.answer_key_ddx || [st.answer_key_ddx1, st.answer_key_ddx2, st.answer_key_ddx3].filter(Boolean).join(", ") || null,
