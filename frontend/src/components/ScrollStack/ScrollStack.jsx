...
export const ScrollStackItem = ({ children, itemClassName = '', className = '', ...props }) => (
  <div className={`scroll-stack-card ${itemClassName} ${className}`.trim()} {...props}>
    {children}
  </div>
);
...
const ScrollStack = ({
    ...
    (element, index) => {
      ...
      if (useWindowScroll) {
        ...
        const currentTranslateY =
          typeof index === 'number' && lastTransformsRef.current.has(index)
            ? lastTransformsRef.current.get(index).translateY
            : 0;
        const currentScrollY =
          window.scrollY || window.pageYOffset || document.documentElement.scrollTop || 0;
        return rect.top + currentScrollY - currentTranslateY;
      } else {
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const pinEnd = endElementTop ? endElementTop - containerHeight / 2 : pinStart + 800;
