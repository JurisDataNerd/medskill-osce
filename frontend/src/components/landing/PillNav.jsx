...
import { ArrowRight, LayoutDashboard, LogOut, Menu, X, ChevronDown } from "lucide-react";
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 px-3 sm:px-6 pt-3 sm:pt-4 ${className}`}>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          {/* Right Auth Button / Profile Dropdown */}
          <div className="hidden md:flex items-center gap-3">
            {!user ? (
              <Link
                to="/login"
                className="inline-flex items-center gap-2 rounded-full px-6 py-3 text-xs sm:text-sm font-extrabold text-white shadow-md transition-all duration-200 hover:scale-105"
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2.5 rounded-full border border-amber-200/80 bg-amber-50/80 px-4 py-2 text-slate-800 transition hover:bg-amber-100 cursor-pointer"
                >
                  <div
                    className="flex h-8 w-8 items-center justify-center rounded-full font-black text-white text-xs shadow-xs"
                    style={{ backgroundColor: baseColor }}
                  >
                    {user.email?.charAt(0).toUpperCase()}
                  </div>
                  <span className="text-xs font-bold text-[#0D3A68]">
                    {user.email?.split("@")[0]}
                  </span>
                  <ChevronDown
                    size={14}
                ...
                <AnimatePresence>
                  {dropdownOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: 8 }}
                      className="absolute right-0 mt-3 w-60 overflow-hidden rounded-2xl border border-blue-100 bg-white p-2 shadow-2xl z-50 space-y-1"
                    >
                      <div className="px-3 py-2 border border-slate-100 bg-slate-50 rounded-xl">
                        <p className="text-[10px] uppercase font-bold text-slate-400">Akun Terdaftar:</p>
                      ...
                      <Link
                        ...
                        className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-xs font-extrabold text-white shadow-xs transition"
                        style={{ backgroundColor: baseColor }}
                      >
          ...
          <div className="flex lg:hidden items-center">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-[#0D3A68] cursor-pointer"
            >
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="overflow-hidden border-t border-slate-200/60 mt-3 pt-3 lg:hidden bg-white rounded-2xl p-3 shadow-md"
            >
              <div className="flex flex-col gap-2 text-xs font-bold">
                {items.map((item, idx) => (
                  <button
                    key={idx}
                    onClick={() => scrollToSection(item.href)}
                    className="text-left py-2.5 px-3.5 rounded-xl hover:bg-amber-50 text-slate-800 hover:text-[#0D3A68]"
                  >
                    {item.label}
                  </button>
                ))}
                ...
                <div className="pt-2 border-t border-slate-100">
                  {!user ? (
                    <Link
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl py-3 text-xs font-extrabold text-white"
                      style={{ backgroundColor: baseColor }}
                    >
                      <span>Login Portal</span>
                      <ArrowRight size={14} />
                    ...
                    <button
                      onClick={handleLogout}
                      className="flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 py-2.5 text-xs font-bold text-red-600"
                    >
                      <LogOut size={14} />
                      <span>Keluar</span>
                    </button>
